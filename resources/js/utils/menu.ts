...
const getPackageMenuItems = (userRoles: string[], activatedPackages: string[], t: (key: string) => string): NavItem[] => {
    ...
    const allModules = import.meta.glob('../../../packages/automas/*/src/Resources/js/menus/*.ts', { eager: true });
    ...
    activatedPackages.forEach(packageName => {
        const menuPath = `../../../packages/automas/${packageName}/src/Resources/js/menus/${menuType}.ts`;
