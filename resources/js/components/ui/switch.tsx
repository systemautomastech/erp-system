...
>(({ className, size = "default", ...props }, ref) => {
  const isSmall = size === "sm" || className?.includes("scale-") || className?.includes("h-4") || className?.includes("h-5");

  return (
    <SwitchPrimitives.Root
      className={`peer inline-flex flex-row ${
    ...
    >
      <SwitchPrimitives.Thumb
        ...
        } rounded-full bg-background shadow-lg ring-0 transition-transform data-[state=unchecked]:translate-x-0`}
      />
    </SwitchPrimitives.Root>
  )
})
