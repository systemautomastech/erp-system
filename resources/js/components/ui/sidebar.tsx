...
>(
  (
    ...
    // Auto-detect side based on RTL - reactive to changes
    const [effectiveSide, setEffectiveSide] = React.useState(() => {
      const isRTL = document.documentElement.dir === 'rtl'
      return side ?? (isRTL ? 'right' : 'left')
    })
    ...
    React.useEffect(() => {
      const observer = new MutationObserver(() => {
        const isRTL = document.documentElement.dir === 'rtl'
        setEffectiveSide(side ?? (isRTL ? 'right' : 'left'))
      })
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      size="icon"
      className={cn(
        "h-8 w-8 text-black dark:text-white hover:bg-accent hover:text-black dark:hover:text-white transition-colors",
        className
      )}
      onClick={(event) => {
        onClick?.(event)
...
>(({ className, ...props }, ref) => {
  return (
    <Input
      ...
      data-sidebar="input"
      className={cn(
        "h-8 w-full bg-background shadow-none focus-visible:ring-2 focus-visible:ring-sidebar-ring transition-all",
        className
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ...
      className={cn(
        "flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto overflow-x-hidden sidebar-scrollbar",
        className
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => (
  <ul
    ...
    className={cn(
      "mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-l-2 border-sidebar-border pl-2.5 pr-0 py-0.5",
      "group-data-[collapsible=icon]:hidden",
      className
    )}
    {...props}
