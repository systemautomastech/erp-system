...
export default function Print() {
    ...
    const customFields = useFormFields('getCustomFields', { ...invoice, module: 'General', sub_module: 'Purchase Invoice', id: invoice.id, isPrint: true }, () => {}, {}, 'view', t);
    useEffect(() => {
                        ...
                        <div className="text-sm space-y-1">
                            ...
                            {invoice.vendor_details?.billing_address && (
                                <>
                                    ...
                                    <p>{invoice.vendor_details.billing_address.city}, {invoice.vendor_details.billing_address.state} {invoice.vendor_details.billing_address.zip_code}</p>
                                </>
                            )}
                        </div>
                    </div>
                        ...
                        <div className="text-sm space-y-1">
                            {invoice.vendor_details?.shipping_address ? (
                                <>
                                    ...
                                    <p>{invoice.vendor_details.shipping_address.city}, {invoice.vendor_details.shipping_address.state} {invoice.vendor_details.shipping_address.zip_code}</p>
                                </>
                            ) : (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            {customFields.map((field) => (
                                <div key={field.id} className="space-y-1">
                                    <div className="font-semibold text-gray-700">{(field as any).label || (field as any).name || 'Custom Field'}:</div>
