...
export default function StorageSettings({ userSettings, auth }: StorageSettingsProps) {
  ...
  const saveSettings = () => {
    if (!settings.allowedFileTypes.trim()) {
      toast.error(t('At least one file type must be selected'));
      return;
    }
    ...
    try {
      router.post(route('settings.storage.update'), {
        ...
        onError: (errors) => {
          setIsLoading(false);
          const errorMessage = errors.error || Object.values(errors).join(', ') || t('Failed to save storage settings');
          toast.error(errorMessage);
        }
      });
    } catch (error) {
      setIsLoading(false);
      toast.error(t('Failed to update storage settings'));
    }
  };
