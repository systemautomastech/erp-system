...
<body>
    <!-- Sleek Centered Floating Action Pill Dock (Light Theme rounded-md) -->
    <header class="fixed top-3 sm:top-4 left-1/2 -translate-x-1/2 z-50 flex items-center justify-between gap-2 sm:gap-3 max-w-[92vw] sm:max-w-none px-3 sm:px-4 py-1.5 print:hidden bg-white/95 hover:bg-white backdrop-blur-md rounded-md shadow-md border border-slate-200/90 transition-all duration-200">
        <div class="flex items-center gap-1.5 sm:gap-2 pl-0.5 pr-2 border-r border-slate-200 shrink-0">
            <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
            <span class="text-[11px] sm:text-xs font-bold text-slate-800 tracking-tight truncate max-w-[110px] sm:max-w-none">#{{ $invoice->invoice_number }}</span>
        </div>
        <div class="flex items-center gap-1.5 shrink-0">
            <button onclick="window.print()"
                class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:text-slate-900 bg-slate-100 hover:bg-slate-200 border border-slate-200/60 rounded-md transition-all cursor-pointer">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5 text-slate-600" viewBox="0 0 24 24" fill="none"
            ...
            <a href="?download=pdf"
                ...
                class="inline-flex items-center justify-center p-2 text-xs font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-md shadow-xs transition-all cursor-pointer">
    ...
    <main class="invoice-viewer-wrapper">
        ...
        <div class="mobile-invoice-view">
            <div class="bg-white rounded-xl p-5 sm:p-6 shadow-sm border border-slate-200 space-y-6 relative overflow-hidden">
                <!-- Mobile Header: Invoice Details & QR First, Company Info After -->
                <div class="flex flex-col sm:flex-row items-start justify-between gap-4 pb-5 border-b border-slate-200">
                    <!-- Top/Right: Invoice Details & QR Code -->
                    <div class="flex items-start justify-between sm:justify-end gap-2.5 w-full sm:w-auto text-left sm:text-right">
                        <div>
                        ...
                        @if(!empty($qrCodeSvg))
                            <div class="shrink-0 p-1.5 bg-white border border-slate-300 rounded-lg shadow-sm inline-block self-center">
                                <div class="w-20 h-20 sm:w-24 sm:h-24 [&>svg]:w-full [&>svg]:h-full">
                                    {!! $qrCodeSvg !!}
                                </div>
                            </div>
                        @endif
                    ...
                    <!-- Bottom/Left: Company Info / Name & Logo -->
                    <div class="space-y-1 w-full sm:w-auto pt-2 sm:pt-0 border-t border-slate-100 sm:border-t-0">
                        @if($showLogo && $logoUrl)
                            <div class="mb-1.5">
                        ...
                        @endif
                        @if(!empty($companySettings['company_address']))
                            <p class="text-[11px] text-slate-500 sm:max-w-[200px] leading-tight">{{ $companySettings['company_address'] }}</p>
                        @endif
                        @if(!empty($companySettings['company_telephone']))
