...
export default function Create({ onSuccess }: CreateWarningProps) {
    const { users, allUsers, warningtypes, auth } = usePage<any>().props;
        ...
        <DialogContent>
            ...
            <form onSubmit={submit} className="space-y-4">
                ...
                <div>
                    ...
                    {warningtypes?.length === 0 && auth?.user?.permissions?.includes('create-warning-types') && (
                        <p className="text-xs text-gray-500 mt-1">
                            {t('Create warning type here.')} <button type="button" onClick={() => router.get(route('hrm.warning-types.index'))} className="text-blue-600 hover:underline">{t('Create warning type')}</button>
                        </p>
                    )}
                </div>
