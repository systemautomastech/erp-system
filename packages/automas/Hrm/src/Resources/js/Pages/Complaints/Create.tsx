...
export default function Create({ onSuccess }: CreateComplaintProps) {
    const { employees, allEmployees, complaintTypes, auth } = usePage<any>().props;
        ...
        <DialogContent>
            ...
            <form onSubmit={submit} className="space-y-4">
                ...
                <div>
                    ...
                    {complaintTypes?.length === 0 && auth?.user?.permissions?.includes('create-complaint-types') && (
                        <p className="text-xs text-gray-500 mt-1">
                            {t('Create complaint type here.')} <button type="button" onClick={() => router.get(route('hrm.complaint-types.index'))} className="text-blue-600 hover:underline">{t('Create complaint type')}</button>
                        </p>
                    )}
                </div>
