...
export default function Create({ onSuccess }: CreateLeaveApplicationProps) {
    const { employees, leavetypes,auth } = usePage<any>().props;
        ...
        <DialogContent>
            ...
            <form onSubmit={submit} className="space-y-4">
                ...
                <div>
                    ...
                    {leavetypes?.length === 0 && auth?.user?.permissions?.includes('create-leave-types') && (
                        <p className="text-xs text-gray-500 mt-1">
                            {t('Create leave type here.')} <button type="button" onClick={() => router.get(route('hrm.leave-types.index'))} className="text-blue-600 hover:underline">{t('Create leave type')}</button>
                        </p>
                    )}
                </div>
