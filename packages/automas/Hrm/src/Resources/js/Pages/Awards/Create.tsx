...
export default function Create({ onSuccess }: CreateAwardProps) {
    const { employees, awardTypes, auth } = usePage<any>().props;
        ...
        <DialogContent>
            ...
            <form onSubmit={submit} className="space-y-4">
                ...
                <div>
                    ...
                    {awardTypes?.length === 0 && auth?.user?.permissions?.includes('create-award-types') && (
                        <p className="text-xs text-gray-500 mt-1">
                            {t('Create award type here.')} <button type="button" onClick={() => router.get(route('hrm.award-types.index'))} className="text-blue-600 hover:underline">{t('Create award type')}</button>
                        </p>
                    )}
                </div>
