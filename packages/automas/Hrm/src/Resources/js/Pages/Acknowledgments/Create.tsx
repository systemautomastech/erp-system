...
export default function Create({ onSuccess }: CreateAcknowledgmentProps) {
    const { users, hrmdocuments,auth } = usePage<any>().props;
        ...
        <DialogContent>
            ...
            <form onSubmit={submit} className="space-y-4">
                ...
                <div>
                    ...
                    {hrmdocuments?.length === 0 && auth?.user?.permissions?.includes('create-hrm-documents') && (
                        <p className="text-xs text-gray-500 mt-1">
                            {t('Create document here.')} <button type="button" onClick={() => router.get(route('hrm.documents.index'))} className="text-blue-600 hover:underline">{t('Create document')}</button>
                        </p>
                    )}
                </div>
