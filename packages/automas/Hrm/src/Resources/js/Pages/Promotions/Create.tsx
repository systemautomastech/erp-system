...
import { CreatePromotionProps, CreatePromotionFormData } from './types';
import { usePage, router } from '@inertiajs/react';
import { useEffect, useState } from 'react';

export default function Create({ onSuccess }: CreatePromotionProps) {
    const { employees, branches, departments, designations, auth } = usePage<any>().props;
    const [filteredDepartments, setFilteredDepartments] = useState([]);
    const [filteredDesignations, setFilteredDesignations] = useState([]);

    const { t } = useTranslation();
    const { data, setData, post, processing, errors } = useForm<CreatePromotionFormData>({
        employee_id: '',
        current_branch_id: '',
    ...
    const reasonAI = useFormFields('aiField', data, setData, errors, 'create', 'reason', 'Reason', 'hrm', 'promotion');
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('hrm.promotions.store'), {
            onSuccess: () => {
                onSuccess();
            }
        });
    ...
    return (
        <DialogContent>
            <DialogHeader>
                <DialogTitle>{t('Create Promotion')}</DialogTitle>
            </DialogHeader>
            <form onSubmit={submit} className="space-y-4">
                ...
                <div>
                    ...
                    {(() => {
                        const branchDepartments = data.current_branch_id ? departments.filter(dept => dept.branch_id.toString() === data.current_branch_id) : [];
                        return ((data.current_branch_id && branchDepartments.length === 0) || (!data.current_branch_id && departments?.length === 0)) && auth?.user?.permissions?.includes('create-departments');
                    })() && (
                        <p className="text-xs text-gray-500 mt-1">
                ...
                <div>
                    ...
                    {(() => {
                        const departmentDesignations = data.current_department_id ? designations.filter(desig => desig.department_id.toString() === data.current_department_id) : [];
                        return ((data.current_department_id && departmentDesignations.length === 0) || (!data.current_department_id && designations?.length === 0)) && auth?.user?.permissions?.includes('create-designations');
                    })() && (
                        <p className="text-xs text-gray-500 mt-1">
