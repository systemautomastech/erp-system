...
import { EditHolidayProps, EditHolidayFormData } from './types';
import { usePage } from '@inertiajs/react';
import { useEffect, useState } from 'react';
import axios from 'axios';

export default function EditHoliday({ holiday, onSuccess }: EditHolidayProps) {
    const { holidayTypes } = usePage<any>().props;

    const { t } = useTranslation();
    const { data, setData, put, processing, errors } = useForm<EditHolidayFormData>({
        name: holiday.name ?? '',
    ...
    return (
        <DialogContent>
            <DialogHeader>
                <DialogTitle>{t('Edit Holiday')}</DialogTitle>
            </DialogHeader>
