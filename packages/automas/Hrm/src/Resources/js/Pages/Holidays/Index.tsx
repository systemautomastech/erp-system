                ...
                <div className="flex gap-1">
                    <TooltipProvider>
                        {auth.user?.permissions?.includes('view-holidays') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => setViewingItem(holiday)} className="h-8 w-8 p-0 text-green-600 hover:text-green-700">
                        ...
                        {auth.user?.permissions?.includes('edit-holidays') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => openModal('edit', holiday)} className="h-8 w-8 p-0 text-blue-600 hover:text-blue-700">
                    ...
                    <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="relative">
                                ...
                                />
                                {(() => {
                                    const activeFilters = [filters.holiday_type_id, filters.start_date, filters.end_date, filters.is_paid].filter(f => f !== '' && f !== null && f !== undefined).length;
                ...
                {showFilters && (
                    <CardContent className="p-6 bg-blue-50/30 border-b">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">{t('Holiday Type')}</label>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">{t('Paid')}</label>
                                <Select value={filters.is_paid} onValueChange={(value) => setFilters({...filters, is_paid: value})}>
                                    <SelectTrigger>
                                        <SelectValue placeholder={t('Filter by Paid Status')} />
                                    </SelectTrigger>
                                    <SelectContent>
                        ...
                        <DataTable
                            ...
                            emptyState={
                                <NoRecordsFound
                                    ...
                                    hasFilters={!!(filters.name || filters.holiday_type_id || filters.start_date || filters.end_date || filters.is_paid)}
