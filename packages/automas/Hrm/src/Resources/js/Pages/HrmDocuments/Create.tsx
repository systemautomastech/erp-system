...
export default function Create({ onSuccess }: CreateDocumentProps) {
    const { documentcategories, users,auth } = usePage<any>().props;
            ...
            <form onSubmit={submit} className="space-y-4">
                ...
                <div>
                    ...
                    <InputError message={errors.document_category_id} />
                     {documentcategories?.length === 0 && auth?.user?.permissions?.includes('create-document-categories') && (
                        <p className="text-xs text-gray-500 mt-1">
                            {t('Create document category here.')} <button type="button" onClick={() => router.get(route('hrm.document-categories.index'))} className="text-blue-600 hover:underline">{t('Create document category')}</button>
                        </p>
                    )}
                </div>
