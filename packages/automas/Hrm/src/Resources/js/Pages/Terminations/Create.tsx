...
export default function Create({ onSuccess }: CreateTerminationProps) {
    const { users, terminationtypes, auth } = usePage<any>().props;
        ...
        <DialogContent>
            ...
            <form onSubmit={submit} className="space-y-4">
                <div>
                    ...
                    {users?.length === 0 && auth?.user?.permissions?.includes('create-employees') && (
                ...
                <div>
                    ...
                    {terminationtypes?.length === 0 && auth?.user?.permissions?.includes('create-termination-types') && (
                        <p className="text-xs text-gray-500 mt-1">
                            {t('Create termination type here.')} <button type="button" onClick={() => router.get(route('hrm.termination-types.index'))} className="text-blue-600 hover:underline">{t('Create termination type')}</button>
                        </p>
                    )}
                </div>
