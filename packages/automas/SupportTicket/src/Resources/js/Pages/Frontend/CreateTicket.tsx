...
export default function CreateTicket({ categories, allFields, customFields }: CreateTicketProps) {
    const { t } = useTranslation();
    const { flash, supportTicketSettings, userSlug } = usePage().props as any;
    const [showSuccess, setShowSuccess] = useState(false);
    const [selectedFiles, setSelectedFiles] = useState<File[]>([]);

    // Extract settings from middleware
    const titleSections = supportTicketSettings?.title_sections || {};
    const supportInformation = supportTicketSettings?.support_information || {};
    const quickLinks = supportTicketSettings?.quick_links || [];
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('support-ticket.store',[userSlug]), {
            onSuccess: (response) => {
                // Clear form data completely
                setData({
                    name: '',
                    ...
                    <Card className="mb-6 shadow-lg">
                        ...
                        <CardContent className="p-4">
                            <div className="mb-4 text-center rounded-lg overflow-hidden">
                                <img
                                    src={getImagePath('packages/automas/SupportTicket/src/Resources/assets/images/svg/support-illustration.svg')}
                                    alt="Support"
                            ...
                            <div className="space-y-4">
                                <div className="bg-teal-50 p-3 rounded-lg flex items-start">
                                    <Info className="text-teal-600 mt-1 mr-3 h-5 w-5 flex-shrink-0" />
                                    <div>
                                ...
                                <div className="bg-teal-50 p-3 rounded-lg flex items-start">
                                    <Clock className="text-teal-600 mt-1 mr-3 h-5 w-5 flex-shrink-0" />
                                    <div>
                                ...
                                <div className="bg-teal-50 p-3 rounded-lg flex items-start">
                                    <Phone className="text-teal-600 mt-1 mr-3 h-5 w-5 flex-shrink-0" />
                                    <div>
                            ...
                            {quickLinks && quickLinks.length > 0 ? (
                                quickLinks.map((link, index) => {
                                    return (
                                        ...
                                        >
                                            <SocialLinks icon={link.icon} className="inline mr-3 text-teal-600 h-5 w-5" />
                                            {link.title}
                                        </a>
                                    );
                                })
                        ...
                        <CardContent className="p-4 md:p-8">
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="grid grid-cols-12 gap-4">
                                    {allFields && allFields.length > 0 ? allFields.sort((a, b) => (a.order || 0) - (b.order || 0)).map((field) => {
                                                ...
                                                <div key={field.id} className={colSpan}>
                                                    ...
                                                    <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-teal-400 transition-all duration-300">
                                                        <div className="space-y-1 text-center">
                                                            <img src={getImagePath('packages/automas/SupportTicket/src/Resources/assets/images/svg/file-upload-illustration.svg')} alt="File Upload" className="h-28 mx-auto" style={{
                                                            ...
                                                            <p className="text-xs text-gray-500">{t('PNG, JPG, GIF, PDF up to 10MB')}</p>
