...
import {  Database, Calendar, List, FileCode, Server, Shield, Info, Ticket, Link as LinkIcon} from 'lucide-react';

export default function KnowledgeArticle({ article, relatedArticles }: KnowledgeArticleProps) {
    const { t } = useTranslation();
    const { supportTicketSettings, userSlug } = usePage().props as any;
    const [copiedCode, setCopiedCode] = useState<string | null>(null);

    // Extract settings from middleware
    ...
    return (
        <Layout title={article.title}>
            ...
            <div className="text-sm text-gray-500 mb-4">
                <Link href={route('support-ticket.index', [userSlug])} className="hover:text-teal-600">{t('Home')}</Link>
                <span className="mx-2">/</span>
                <Link href={route('support-ticket.knowledge', [userSlug])} className="hover:text-teal-600">{t('Knowledge Base')}</Link>
                {article.category && (
                    <>
                        <span className="mx-2">/</span>
                    ...
                    <div className="w-full md:w-3/5 text-white mb-8 md:mb-0 md:pr-8">
                        ...
                        <div className="flex flex-wrap items-center gap-4 lg:gap-6 text-sm">
                            <div className="flex items-center">
                                ...
                                <span>{t('Last updated')}: {formatDate(article.updated_at)}</span>
                            </div>
                        </div>
                    </div>
                    <div className="w-full md:w-2/5 flex justify-center">
                      
                            <img src={getImagePath('packages/automas/SupportTicket/src/Resources/assets/images/svg/database-illustration.svg')}
                            ...
                            <CardContent className="p-4 md:p-5">
                                ...
                                <div className="mt-8 pt-6 border-t border-gray-200">
                                    ...
                                    <div className="space-y-3">
                                        <Link href={route('support-ticket.knowledge', [userSlug])} className="flex items-center text-gray-700 hover:text-teal-600 text-sm">
                                            <FileCode className="h-4 w-4 mr-2 text-teal-600" />
                                            <span>{t('Browse All Articles')}</span>
                                        </Link>
                                        <Link href={route('support-ticket.index', [userSlug])} className="flex items-center text-gray-700 hover:text-teal-600 text-sm">
                                            <Server className="h-4 w-4 mr-2 text-teal-600" />
                                            <span>{t('Create Support Ticket')}</span>
                                        </Link>
                                        {featureSettings.faq_is_on === 'on' && (
                                            <Link href={route('support-ticket.faq', [userSlug])} className="flex items-center text-gray-700 hover:text-teal-600 text-sm">
                    ...
                    <div className="w-full lg:w-1/4">
                                ...
                                <div className="space-y-4">
                                    {relatedArticles && relatedArticles.length > 0 ? (
                                        relatedArticles.map((relatedArticle) => (
                                            <Link key={relatedArticle.id} href={route('support-ticket.knowledge.article', [userSlug, relatedArticle.id])} className="block rounded-lg p-4 border border-gray-200 hover:border-teal-600 hover:shadow-md hover:-translate-y-1 transition-all duration-300">
                                                ...
                                                <p className="text-xs text-gray-400 mt-2">{formatDate(relatedArticle.created_at)}</p>
                                            </Link>
                                        ))
                                    ) : (
                        ...
                        <Card className="shadow-md">
                            <CardContent className="p-5">
                                ...
                                <div className="space-y-4">
                                    <Link href={route('support-ticket.knowledge', [userSlug])} className="block rounded-lg p-4 border border-gray-200 hover:border-teal-600 hover:shadow-md hover:-translate-y-1 transition-all duration-300">
