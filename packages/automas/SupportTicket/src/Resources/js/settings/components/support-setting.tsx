...
export default function SupportSettings({ userSettings, fields: initialFields, auth }: SupportSettingsProps) {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState({ isOpen: false, index: -1, fieldName: '' });
  ...
  const removeField = (index: number) => {
    ...
    if (field.custom_id && field.custom_id <= 6) {
      toast.error(t('Cannot delete default fields'));
      return;
    }
    setDeleteConfirm({ isOpen: true, index, fieldName: field.name || t('Untitled Field') });
  };

  const confirmDelete = () => {
    if (deleteConfirm.index >= 0) {
      setFields(prev => prev.filter((_, i) => i !== deleteConfirm.index));
    }
    setDeleteConfirm({ isOpen: false, index: -1, fieldName: '' });
  };
  ...
  const saveSettings = () => {
    ...
    router.post(route('support-ticket.settings.update'), {
      ...
      onError: (errors) => {
        setIsLoading(false);
        const errorMessage = errors.error || Object.values(errors).join(', ') || t('Failed to save support settings');
        toast.error(errorMessage);
      }
    });
  };
  ...
  return (
    <Card>
      ...
      <ConfirmationDialog
        ...
        message={t('Are you sure you want to delete the field "{{fieldName}}"? This action cannot be undone.', { fieldName: deleteConfirm.fieldName })}
