...
import DefaultPageForm, {
    DefaultPageData,
    VariableGroup,
} from "@/components/DefaultPageForm";
...
export default function Edit({ settings, defaultPage }: EditProps) {
    ...
    return (
        <>
            <Head
                title={`${t("Edit Default Page")} : ${defaultPage?.title || ""}`}
            />
            <DefaultPageForm
                mode="edit"
                ...
                updateRoute={route(
                    "quotation-setup.default-pages.update",
                    defaultPage.id,
                )}
                ...
                pageTitle={`${t("Edit Default Page")} : ${defaultPage?.title || ""}`}
            />
        </>
    );
}
