...
interface DefaultPagesIndexProps {
    ...
    defaultPages: QuotationDefaultPageItem[];
}

export default function DefaultPages({ settings, defaultPages = [] }: DefaultPagesIndexProps) {
    const { t } = useTranslation();
    const pageProps = usePage<any>().props;

    const [items, setItems] = useState<QuotationDefaultPageItem[]>(defaultPages);
    const [searchQuery, setSearchQuery] = useState('');
    const [isViewModalOpen, setIsViewModalOpen] = useState(false);
    const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
    const [viewingPage, setViewingPage] = useState<QuotationDefaultPageItem | null>(null);
    const [deletingPage, setDeletingPage] = useState<QuotationDefaultPageItem | null>(null);
    ...
    const handleSaveOrder = async () => {
        ...
        try {
            ...
            const response = await axios.post(route('quotation-setup.default-pages.reorder'), { orders });
    ...
    const openDeleteModal = (page: QuotationDefaultPageItem) => {
    ...
    const handleConfirmDelete = () => {
        ...
        router.delete(route('quotation-setup.default-pages.destroy', deletingPage.id), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const handleToggleStatus = (page: QuotationDefaultPageItem, checked: boolean) => {
        if (page.page_type === 'otc' || page.page_type === 'mrc') {
        ...
        router.put(route('quotation-setup.default-pages.update', page.id), {
            title: page.title,
            is_active: checked,
        }, {
            preserveScroll: true,
    ...
    const handleViewPage = (page: QuotationDefaultPageItem) => {
        if (page.page_type === 'otc' || page.page_type === 'mrc') {
            return;
        }
        setViewingPage(page);
    ...
    return (
        <TooltipProvider delayDuration={150}>
            <div className="space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">
                            {t('Drag cards using the handle to easily sort pages in your desired quotation sequence.')}
                        </p>
                    </div>
                    <div className="flex items-center gap-2">
                        ...
                        <Button asChild size="sm" className="gap-2 shadow-xs">
                            <Link href={route('quotation-setup.default-pages.create')}>
                    ...
                    {filteredPages.length === 0 ? (
                        <div className="col-span-full flex flex-col items-center justify-center py-16 text-center border rounded-xl bg-card shadow-xs">
                            ...
                            <p className="text-xs text-muted-foreground max-w-sm mt-1">
                                {searchQuery ? t('No pages match your search criteria. Try a different search term.') : t('Get started by creating your first default quotation page template.')}
                            </p>
                            {searchQuery ? (
                                <Button
                                    type="button"
                                    ...
                                    <div className="flex items-center justify-between pt-3 mt-3 border-t border-border/50 text-xs">
                                        ...
                                        <div className="flex items-center gap-1">
                                                ...
                                                <Tooltip>
                                                    ...
                                                    <TooltipContent side="top" className="max-w-xs text-xs p-2.5 bg-popover text-popover-foreground border shadow-lg">
                                                        ...
                                                        <p className="leading-relaxed text-muted-foreground">
                                                            {t('Fixed dynamic page. Automatically generated on the quotation page when items are added. You can configure its sorting order here.')}
                                                        </p>
                                                    </TooltipContent>
                                                </Tooltip>
                                            )}
                                            ...
                                            {canManagePage && (
                                                ...
                                                >
                                                    <Link href={route('quotation-setup.default-pages.edit', page.id)}>
                ...
                {/* Reusable Quotation Preview Modal */}
                <PreviewModal
