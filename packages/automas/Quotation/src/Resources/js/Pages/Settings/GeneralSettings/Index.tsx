...
export default function GeneralSettings({ settings }: GeneralSettingsProps) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        settings: {
            ...
            quotation_starting_number: settings?.quotation_starting_number?.toString() ?? '1001',
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex flex-wrap items-start gap-4">
                <div className="space-y-1.5 w-64">
                    <Label className="text-xs">{t('Quotation Prefix & Number')}</Label>
                    <div className="flex items-center rounded-md border border-input bg-background overflow-hidden focus-within:ring-1 focus-within:ring-ring focus-within:border-ring">
                        <div className="w-24 border-r">
                            <Input
                                id="quotation_prefix"
                                value={data.settings.quotation_prefix}
                                onChange={(e) => setData('settings', { ...data.settings, quotation_prefix: e.target.value })}
                                placeholder="QT-"
                        ...
                        <div className="flex-1">
                            <Input
                                ...
                                type="number"
                                value={data.settings.quotation_starting_number}
                                onChange={(e) => setData('settings', { ...data.settings, quotation_starting_number: e.target.value })}
                                placeholder="1001"
                                className="border-0 rounded-none focus-visible:ring-0 focus-visible:ring-offset-0 h-9 text-xs px-2"
                            />
                        </div>
                    ...
                    <p className="text-[11px] text-muted-foreground pt-0.5">
                        {t('Preview: {{prefix}}{{number}}', { prefix: data.settings.quotation_prefix, number: data.settings.quotation_starting_number })}
                    </p>
                </div>
