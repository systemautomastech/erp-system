...
use Automas\Quotation\Http\Requests\StoreQuotationRequest;
use Automas\Quotation\Http\Requests\UpdateQuotationRequest;
use App\Models\User;
...
class QuotationController extends Controller
{
    ...
    public function update(UpdateQuotationRequest $request, SalesQuotation $quotation)
    {
        if (Auth::user()->can('edit-quotations') && $quotation->created_by == creatorId()) {
            // Check if this is a revision creation request
            if ($request->is_creating_revision) {
                if ($quotation->status === 'draft') {
                    return redirect()->route('quotations.index')->with('error', __('Cannot create version of draft quotation.'));
                }

                // Create new revision
                $totals = $this->calculateTotals($request->items);
                
                $newRevision = new SalesQuotation();
                $newRevision->parent_quotation_id = $quotation->id;
                $newRevision->revision_number = $quotation->revision_number + 1;
                $newRevision->quotation_date = $request->invoice_date;
                $newRevision->due_date = $request->due_date;
                $newRevision->customer_id = $request->customer_id;
                $newRevision->warehouse_id = $request->warehouse_id;
                $newRevision->payment_terms = $request->payment_terms;
                $newRevision->notes = $request->notes;
                $newRevision->subtotal = $totals['subtotal'];
                $newRevision->tax_amount = $totals['tax_amount'];
                $newRevision->discount_amount = $totals['discount_amount'];
                $newRevision->total_amount = $totals['total_amount'];
                $newRevision->status = 'draft';
                $newRevision->converted_to_invoice = false;
                $newRevision->invoice_id = null;
                $newRevision->creator_id = Auth::id();
                $newRevision->created_by = creatorId();
                $newRevision->save();

                $this->createQuotationItems($newRevision->id, $request->items);

                return redirect()->route('quotations.index')->with('success', __('New quotation version created successfully.'));
            }
