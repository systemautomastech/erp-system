...
export default function Index() {
    ...
    const handleFilter = () => {
        ...
        router.get(route('salesorder.orders.index'), {...filterParams, per_page: perPage, sort: sortField, direction: sortDirection, view: viewMode}, {
            preserveState: true,
            replace: true
        });
    };
    ...
    const handleSort = (field: string) => {
        ...
        router.get(route('salesorder.orders.index'), {...filters, per_page: perPage, sort: field, direction, view: viewMode}, {
            preserveState: true,
            replace: true
        });
    };
    ...
    const tableColumns = [
        {
            ...
            render: (value: string, order: any) =>
                auth.user?.permissions?.includes('view-sales-orders') ? (
                    <span className="text-blue-600 hover:text-blue-700 cursor-pointer" onClick={() => router.get(route('salesorder.orders.show', order.id))}>#{order.order_number || order.order_number || value}</span>
                ) : (
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        <h3 className="font-semibold text-sm text-gray-900">
                                                            {auth.user?.permissions?.includes('view-sales-orders') ? (
                                                                <span className="text-blue-600 hover:text-blue-700 cursor-pointer" onClick={() => router.get(route('salesorder.orders.show', order.id))}>#{order.order_number || order.id}</span>
                                                            ) : (
