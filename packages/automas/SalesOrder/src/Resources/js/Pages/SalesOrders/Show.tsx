...
export default function Show() {
    ...
    const { salesOrder, orderItems = [], quotation, auth } = usePage<ShowSalesOrderProps>().props;
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <div className="space-y-1 text-sm">
                                    ...
                                    <div className="flex justify-between"><span className="text-muted-foreground">{t('Assigned User')}</span> <span>{salesOrder.assign_user?.name || (salesOrder.assigned_users && salesOrder.assigned_users[0]?.name) || t('Unassigned')}</span></div>
                                    {salesOrder.quote_id && (
                                        <div className="flex justify-between"><span className="text-muted-foreground">{t('Quote')}</span> <span>{quotation?.quotation_number || salesOrder.quote?.name || `#${salesOrder.quote_id}`}</span></div>
                                    )}
                                </div>
                            </div>
                        </div>
                                        ...
                                        <TooltipProvider>
                                            <Tooltip delayDuration={0}>
                                                <TooltipTrigger asChild>
                                                    <Button
                                                        size="sm"
                                                        onClick={() => router.post(route('salesorder.orders.convert', salesOrder.id), {}, {
                                                            onSuccess: () => {
                                                                router.reload();
                                                            }
                                                        })}
