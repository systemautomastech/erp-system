...
interface EditPosCounterFormData {
    name: string;
    code: string;
    status: boolean;
    description: string;
...
export default function Edit({ counter, onSuccess }: EditProps) {
    const { t } = useTranslation();

    const { data, setData, put, processing, errors } = useForm<EditWarehouseFormData>(counter);

    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('pos.billing-counters.update', counter.id), {
            onSuccess: () => {
                onSuccess?.();
            },
        });
    ...
    return (
        <DialogContent className="max-w-lg">
            <DialogHeader>
                <DialogTitle>{t('Edit POS Billing Counter')}</DialogTitle>
            </DialogHeader>
