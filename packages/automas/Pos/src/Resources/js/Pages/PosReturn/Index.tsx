...
import { PosReturn, PosReturnFilters } from './types';
import { getStatusBadgeClasses } from './utils';

interface PosReturnIndexProps {
    returns: {
        data: PosReturn[];
        links: any[];
        meta: any;
...
export default function Index() {
    ...
    const { returns, customers, warehouses, filters: initialFilters, auth } = usePage<PosReturnIndexProps>().props;
    const urlParams = new URLSearchParams(window.location.search);

    const [filters, setFilters] = useState<PosReturnFilters>({
    ...
    const pageButtons = usePageButtons('posReturnBtn', 'POS Return data');

    const { deleteState, openDeleteDialog, closeDeleteDialog, confirmDelete } = useDeleteHandler({
        routeName: 'pos.returns.destroy',
        defaultMessage: t('Are you sure you want to delete this POS return?')
    });

    const handleFilter = () => {
        router.get(route('pos.returns.index'), {...filters, per_page: perPage, sort: sortField, direction: sortDirection, view: viewMode}, {
            preserveState: true,
            replace: true
        });
    };
    ...
    const handleSort = (field: string) => {
        ...
        router.get(route('pos.returns.index'), {...filters, per_page: perPage, sort: field, direction, view: viewMode}, {
            preserveState: true,
            replace: true
        });
    };
    ...
    const clearFilters = () => {
        ...
        router.get(route('pos.returns.index'), {per_page: perPage, view: viewMode});
    };
    ...
    const tableColumns = [
        {
            ...
            sortable: true,
            render: (value: string, returnItem: PosReturn) =>
                auth.user?.permissions?.includes('view-pos-returns') ? (
                    <span className="text-blue-600 hover:text-blue-700 cursor-pointer" onClick={() => router.get(route('pos.returns.show', returnItem.id))}>{value}</span>
                ) : (
                    value
                )
        },
        ...
        },
        {
            key: 'items',
            header: t('Items'),
            render: (value: any, returnItem: PosReturn) => (
        ...
        ...(auth.user?.permissions?.some((p: string) => ['view-pos-returns', 'delete-pos-returns', 'approve-pos-returns', 'complete-pos-returns'].includes(p)) ? [{
            key: 'actions',
            header: t('Actions'),
            render: (_: any, returnItem: PosReturn) => (
                <div className="flex gap-1">
                    <TooltipProvider>
                        {returnItem.status === 'draft' && (
                            <>
                            {auth.user?.permissions?.includes('approve-pos-returns') && (
                                <Tooltip delayDuration={0}>
                                    <TooltipTrigger asChild>
                                        <Button
                                            variant="ghost"
                                            size="sm"
                                            onClick={() => router.post(route('pos.returns.approve', returnItem.id))}
                        ...
                        {returnItem.status === 'approved' && auth.user?.permissions?.includes('complete-pos-returns') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button
                                        variant="ghost"
                                        size="sm"
                                        onClick={() => router.post(route('pos.returns.complete', returnItem.id))}
                            ...
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button
                                        variant="ghost"
                                        size="sm"
                                        onClick={() => router.get(route('pos.returns.show', returnItem.id))}
                        ...
                        {returnItem.status === 'draft' && (
                            <>
                                {auth.user?.permissions?.includes('delete-pos-returns') && (
                                    <Tooltip delayDuration={0}>
                                        <TooltipTrigger asChild>
                                            <Button
                                                variant="ghost"
    ...
    return (
        <AuthenticatedLayout
            breadcrumbs={[{label: t('POS Returns')}]}
            pageTitle={t('Manage POS Returns')}
            pageActions={
                <div className="flex gap-2">
                    <TooltipProvider>
                        {auth.user?.permissions?.includes('create-pos-returns') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button size="sm" onClick={() => router.visit(route('pos.returns.create'))}>
                            ...
                            <div className="min-w-[800px]">
                                <DataTable
                                    ...
                                    emptyState={
                                        <NoRecordsFound
                                            icon={XCircle}
                                            title={t('No POS returns found')}
                                            description={t('Get started by creating your first POS return.')}
                                            hasFilters={!!(filters.search || filters.customer_id || filters.status || filters.warehouse_id || filters.date_range)}
                                            onClearFilters={clearFilters}
                                            createPermission="create-pos-returns"
                                            onCreateClick={() => router.visit(route('pos.returns.create'))}
                                            createButtonText={t('Create POS Return')}
                                            className="h-auto"
                                        />
                                    }
                                />
                                        ...
                                        <Card key={returnItem.id} className="border border-gray-200 flex flex-col">
                                            <div className="p-4 flex-1">
                                                <div className="flex items-center justify-between mb-3">
                                                    {auth.user?.permissions?.includes('view-pos-returns') ? (
                                                        <h3 className="font-semibold text-base text-blue-600 hover:text-blue-700 cursor-pointer" onClick={() => router.get(route('pos.returns.show', returnItem.id))}>{returnItem.return_number}</h3>
                                                    ) : (
                                            ...
                                            <div className="flex items-center justify-between p-3 border-t bg-gray-50/50">
                                                <div className="flex gap-1">
                                                    <TooltipProvider>
                                                        {returnItem.status === 'draft' && auth.user?.permissions?.includes('approve-pos-returns') && (
                                                            <Tooltip delayDuration={0}>
                                                                <TooltipTrigger asChild>
                                                                    <Button variant="ghost" size="sm" onClick={() => router.post(route('pos.returns.approve', returnItem.id))} className="h-8 w-8 p-0 text-gray-600 hover:text-gray-700">
                                                                        <CheckCircle className="h-4 w-4" />
                                                                    </Button>
                                                                </TooltipTrigger>
                                                        ...
                                                        {returnItem.status === 'approved' && auth.user?.permissions?.includes('complete-pos-returns') && (
                                                            <Tooltip delayDuration={0}>
                                                                <TooltipTrigger asChild>
                                                                    <Button variant="ghost" size="sm" onClick={() => router.post(route('pos.returns.complete', returnItem.id))} className="h-8 w-8 p-0 text-blue-600 hover:text-blue-700">
                                                                        <Check className="h-4 w-4" />
                                                                    </Button>
                                                                </TooltipTrigger>
                                                ...
                                                <div className="flex gap-1">
                                                    <TooltipProvider>
                                                        {auth.user?.permissions?.includes('view-pos-returns') && (
                                                            <Tooltip delayDuration={0}>
                                                                <TooltipTrigger asChild>
                                                                    <Button variant="ghost" size="sm" onClick={() => router.get(route('pos.returns.show', returnItem.id))} className="h-8 w-8 p-0 text-green-600 hover:text-green-700">
                                                                        <Eye className="h-4 w-4" />
                                                                    </Button>
                                                                </TooltipTrigger>
                                                        ...
                                                        {returnItem.status === 'draft' && auth.user?.permissions?.includes('delete-pos-returns') && (
                                                            <Tooltip delayDuration={0}>
                                                                <TooltipTrigger asChild>
