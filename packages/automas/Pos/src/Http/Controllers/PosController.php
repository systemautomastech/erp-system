...
use Automas\Pos\Http\Requests\StorePosRequest;
...
use Automas\Pos\Traits\getDiscountFromTrait;
        ...
        $products = $query->get()
            ->map(function ($product) {
                ...
                if ($product->tax_ids && is_array($product->tax_ids) && !empty($product->tax_ids)) {
                    $taxes = \Automas\ProductService\Models\ProductServiceTax::whereIn('id', $product->tax_ids)
                        ->where('created_by', creatorId())
