...
export default function Create() {
    ...
    const { projects, customers } = usePage<CreateProps>().props;
    useFlashMessages();
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        payment_terms: '',
        notes: '',
        items: [{ milestone_id: '', price: '', discount_percentage: '0', discount_amount: 0, total_amount: 0 }] as PaymentItem[],
    });
    ...
    useEffect(() => {
        if (data.project_id) {
            ...
            if (project && project.clients && project.clients.length > 0) {
                setData('customer_id', project.clients[0].id.toString());
            }
        } else {
            setMilestones([]);
        }
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('project-payments.store'));
    };
    ...
    return (
        <AuthenticatedLayout
            ...
            pageTitle={t('Create Project Payment')}
        >
            <Head title={t('Create Project Payment')} />
