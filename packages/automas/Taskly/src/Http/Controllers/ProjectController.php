...
use Automas\Taskly\Events\CreateProject;
use Automas\Taskly\Events\CreateProjectMilestone;
use Automas\Taskly\Events\DestroyProject;
use Automas\Taskly\Events\DestroyProjectMilestone;
use Automas\Taskly\Events\ProjectDeleteClient;
use Automas\Taskly\Events\ProjectDeleteShareToClient;
use Automas\Taskly\Events\ProjectInviteMember;
use Automas\Taskly\Events\ProjectShareToClient;
use Automas\Taskly\Events\UpdateProject;
use Automas\Taskly\Events\UpdateProjectMilestone;
use Automas\Taskly\Models\Project;
use Automas\Taskly\Http\Requests\UpdateMilestoneRequest;
use Automas\Taskly\Http\Requests\StoreMilestoneRequest;
use Automas\Taskly\Http\Requests\StoreProjectRequest;
use Automas\Taskly\Http\Requests\UpdateProjectRequest;
use Automas\Taskly\Models\ActivityLog;
use Automas\Taskly\Models\ProjectMilestone;
use Automas\Taskly\Models\ProjectTask;
use Automas\Taskly\Models\ProjectBug;
use Automas\Taskly\Models\ProjectClient;
use Automas\Taskly\Models\ProjectUser;
use Automas\Taskly\Models\ProjectFile;
...
class ProjectController extends Controller
{
    public function index()
    {
        if (Auth::user()->can('manage-project')) {
            $items = Project::query()
                ...
                ->with(['teamMembers:id,name,avatar', 'milestones:id,project_id,cost,status'])
                ->where(function($q) {
            ...
            $items->getCollection()->transform(function ($project) {
                ...
                $project->bug_count  = ProjectBug::where('project_id', $project->id)->count();

                // Milestone summary
                $project->milestone_count = $project->milestones->count();
                $project->completed_milestones = $project->milestones->where('status', 'Complete')->count();
                $project->milestone_cost = (float) $project->milestones->sum('cost');
    ...
    public function show(Project $project)
    {
        if (Auth::user()->can('view-project') && $project->created_by == creatorId()) {
            ...
            $taskStages = \Automas\Taskly\Models\TaskStage::where('created_by', creatorId())->orderBy('order')->get();
            $chartData = [];
            for ($i = 4; $i >= 0; $i--) {
    ...
    {
        if (Auth::user()->can('view-project') && $project->created_by == creatorId()) {
            $project->load(['teamMembers:id,name']);
            ...
            $tasks = ProjectTask::where('project_id', $project->id)
                ->where('created_by', creatorId())
                ->with(['taskStage:id,name,color', 'milestone:id,title'])
            ...
            return Inertia::render('Taskly/Project/Gantt', [
                'project' => $project->only(['id', 'name', 'start_date', 'end_date']),
                'tasks' => $tasks,
