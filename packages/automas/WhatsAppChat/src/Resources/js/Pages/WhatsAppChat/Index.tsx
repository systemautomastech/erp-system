...
import { MessageCircle, Send, Search, User as UserIcon, Plus, Phone, Edit, Smile, Paperclip, X } from "lucide-react";
import { getImagePath, getAdminSetting } from '@/utils/helpers';
import { EmojiPicker } from '@/components/ui/emoji-picker';
import { WhatsAppChatIndexProps } from '../Contacts/types';
import ChatInitialize from '../Contacts/ChatInitialize';
...
export default function Index() {
    const pageProps = usePage<WhatsAppChatIndexProps & { users: Record<string, string> }>().props;
    const { contacts, auth, users } = pageProps;
    ...
    const [contactsList, setContactsList] = useState<WhatsAppContact[]>(contacts || []);
    ...
    const handleContactSelect = async (contact: any) => {
        ...
        try {
            const response = await axios.post(route('whatsapp-chat.get-chat'), {
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        try {
            await axios.post(route('whatsapp-chat.send-message'), formData, {
                headers: {
    ...
    const openEditNameModal = (contact: WhatsAppContact) => {
    ...
    const handleSaveContactName = async () => {
        ...
        try {
            await axios.post(route('whatsapp-chat.update-name'), {
                contact_id: editingContact.id,
                name: newContactName.trim()
            });

            setSelectedContact(prev => prev?.id === editingContact.id ? {...prev, name: newContactName.trim()} : prev);
            setContactsList(prev => prev.map(c => c.id === editingContact.id ? {...c, name: newContactName.trim()} : c));
            closeEditNameModal();
        } catch (error) {
        }
    };
    ...
    useEffect(() => {
        if (selectedContact && selectedContact.unseen_count > 0) {
            const timer = setTimeout(() => {
                ...
                fetch(route('whatsapp-chat.mark-seen'), {
                    method: 'POST',
                    headers: {
    ...
    useEffect(() => {
        ...
        try {
            ...
            const channelName = `whatsapp-chat-${auth.user.id}`;
            const eventName = `whatsapp-chat-event-${auth.user.id}`;
    ...
    return (
        <AuthenticatedLayout
            breadcrumbs={[{label: t('WhatsApp Chat')}]}
            pageTitle={t('Manage WhatsApp Chat')}
            pageActions={
                <div className="flex gap-2">
                    <TooltipProvider>
                        {auth.user?.permissions?.includes('create-whatsapp-contacts') && (
        ...
        >
                ...
                <Card className="w-80 flex flex-col h-full">
                    <CardHeader className="pb-3 flex-shrink-0">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <h2 className="font-semibold text-gray-900">{t('WhatsApp Contacts')}</h2>
                        </div>
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <p className="text-xs text-gray-500 truncate">{contact.contact_no}</p>
                                            </div>
                    ...
                    <CardHeader className={`pb-3 ${selectedContact ? 'border-b' : 'hidden'}`}>
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1">
                                <h3 className="font-semibold text-gray-900">{selectedContact?.name || selectedContact?.contact_no || ''}</h3>
                                <p className="text-sm text-gray-500">{selectedContact?.contact_no || ''}</p>
                            </div>
                                <TooltipProvider>
                                    <Tooltip delayDuration={0}>
                                        <TooltipTrigger asChild>
                                            <Button
                                                size="sm"
                                                variant="outline"
                                                onClick={() => openEditNameModal(selectedContact)}
                                            >
            ...
            <Dialog open={showEditNameDialog} onOpenChange={setShowEditNameDialog}>
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        <DialogTitle>{t('Add Contact Name')}</DialogTitle>
                    </DialogHeader>
                    <div className="space-y-4">
                        ...
                        <div className="flex justify-end gap-2">
                            <Button type="button" variant="outline" onClick={closeEditNameModal}>
                                {t('Cancel')}
                            </Button>
                            <Button onClick={handleSaveContactName} disabled={!newContactName.trim()}>
                                {t('Save')}
                            </Button>
                        </div>
                    </div>
