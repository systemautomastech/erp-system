...
import { EditSalesCallProps, CreateSalesCallFormData } from './types';
import { useState, useEffect } from 'react';

export default function Edit({ salesCall, onSuccess, users: propUsers, accounts: propAccounts }: EditSalesCallProps) {
    const { t } = useTranslation();
    const { auth } = usePage<any>().props;
    const { data, setData, put, processing, errors } = useForm<CreateSalesCallFormData>({
        name: salesCall.name,
        status: salesCall.status,
        start_date: salesCall.start_date,
        end_date: salesCall.end_date,
        direction: salesCall.direction,
        parent_type: salesCall.parent_type || '',
        parent_id: salesCall.parent_id,

        account_id: salesCall.account_id,
        assigned_user_id: salesCall.assigned_user_id?.toString() || null,
        description: salesCall.description || '',
        attendees_users: salesCall.attendees_users || [],
        attendees_contacts: salesCall.attendees_contacts || [],
    });
    ...
    const formFields = useFormFields('salesCall', data, setData, errors, 'edit');

    useEffect(() => {
        if (!propAccounts) fetchAccounts();
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('sales.calls.update', salesCall.id), {
            onSuccess: () => {
                onSuccess();
            }
        });
    ...
    return (
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={submit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <Label htmlFor="edit_direction">{t('Direction')}</Label>
