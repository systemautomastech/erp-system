...
export default function Create() {
    ...
    const { customers, warehouses, accounts, users, opportunities, contacts, shippingProviders } = usePage<CreateProps>().props;
    ...
    const fromAccount = usePage().props.fromAccount;
    const fromContact = usePage().props.fromContact;
    const fromOpportunity = usePage().props.fromOpportunity;

    const { data, setData, post, processing, errors } = useForm({
        name: '',
        opportunity_id: fromOpportunity || null,
        status: 'Draft',
        account_id: fromAccount || null,
        warehouse_id: '',
        ...
        billing_contact_id: fromContact || null,
    ...
    // Custom fields hook
    const customFields = useFormFields('getCustomFields', { ...data, module: 'Sales', sub_module: 'Quotes' }, setData, errors, 'create', t);
        ...
        if (opportunityId && opportunityId !== 'none') {
            try {
                ...
                setData(prev => ({
                    ...
                    account_id: fromAccount || details.account_id || null
                }));
            } catch (error) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        post(route('sales.quotes.store'), {
            onSuccess: () => {
                if (fromOpportunity) {
                    router.visit(route('sales.opportunities.show', fromOpportunity));
                } else if (fromContact) {
    ...
    // Available contacts - preserve selected contacts even when account changes
    const availableContacts = React.useMemo(() => {
        if (!contacts) return [];
        
        const selectedContactIds = [data.billing_contact_id, data.shipping_contact_id].filter(Boolean);
        const selectedContacts = contacts.filter((contact: any) => selectedContactIds.includes(contact.id));
        
        if (data.account_id) {
            const accountContacts = contacts.filter((contact: any) => contact.account_id === data.account_id);
            const combined = [...selectedContacts, ...accountContacts];
        ...
        return contacts;
    }, [contacts, data.account_id, data.billing_contact_id, data.shipping_contact_id]);
    
    // Show all opportunities including those without an account.
    // When coming from an account/contact context, show opportunities linked to that account
    // PLUS opportunities that have no account assigned (so they are always selectable).
    const filteredOpportunities = (fromAccount || fromContact) && data.account_id
    ...
    return (
        <AuthenticatedLayout
            ...
            pageTitle={t('Create Quote')}
        >
            <Head title={t('Create Quote')} />
                        ...
                        <CardContent>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
                                <div>
                                    ...
                                    <Select value={data.opportunity_id?.toString() || ''} onValueChange={handleOpportunityChange} disabled={!!fromOpportunity}>
                                        <SelectTrigger>
                                ...
                                <div>
                                    ...
                                    <Select value={data.account_id?.toString() || ''} onValueChange={(value) => {
                                        ...
                                        setData(prev => ({
                                            ...
                                            opportunity_id: fromOpportunity ? prev.opportunity_id : null // Don't reset if from opportunity
                                        }));
                                    }} disabled>
                                        <SelectTrigger>
