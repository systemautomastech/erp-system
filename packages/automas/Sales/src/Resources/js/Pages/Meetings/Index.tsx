...
import CreateSalesMeeting from './Create';
import EditSalesMeeting from './Edit';
import { SalesMeetingsIndexProps, SalesMeetingModalState, SalesMeeting } from './types';
import { usePageButtons } from '@/hooks/usePageButtons';

interface SalesMeetingFilters {
    name: string;
    status: string;
    parent_type: string;
    account_id: string;
...
export default function Index() {
    ...
    const { salesMeetings, auth, accounts, users } = usePage<SalesMeetingsIndexProps>().props;
    const urlParams = useMemo(() => new URLSearchParams(window.location.search), []);

    const [filters, setFilters] = useState<SalesMeetingFilters>({
    ...
    const [modalState, setModalState] = useState<SalesMeetingModalState>({
        isOpen: false,
        mode: '',
        data: null
    });
    ...
    useFlashMessages();
    const dropboxBtn = usePageButtons('dropboxBtn', { module: 'Meeting', settingKey: 'Dropbox Meeting' });
    const boxBtn = usePageButtons('boxBtn', { module: 'Meeting', settingKey: 'Box Meeting' });
    const googleDriveButtons = usePageButtons('googleDriveBtn', { module: 'Meeting', settingKey: 'GoogleDrive Meeting' });
    const oneDriveButtons = usePageButtons('oneDriveBtn', { module: 'Meeting', settingKey: 'OneDrive Meeting' });
    ...
    const handleFilter = () => {
        ...
        Object.keys(filters).forEach(key => {
            if (key !== 'date_range' && filters[key as keyof SalesMeetingFilters]) {
                filterParams[key] = filters[key as keyof SalesMeetingFilters];
            }
        });
        ...
        router.get(route('sales.meetings.index'), {...filterParams, per_page: perPage, sort: sortField, sort_direction: sortDirection, view: viewMode}, {
            preserveState: true,
            replace: true
        });
    };
    ...
    const handleSort = (field: string) => {
        ...
        router.get(route('sales.meetings.index'), {...filterParams, per_page: perPage, sort: field, sort_direction: direction, view: viewMode}, {
            preserveState: true,
            replace: true
        });
    };
    ...
    const clearFilters = () => {
        setFilters({ name: '', status: '', parent_type: '', account_id: '', assigned_user_id: '', date_range: '' });
        setSortField('');
        setSortDirection('asc');
        router.get(route('sales.meetings.index'), {per_page: perPage, view: viewMode});
    };

    const openModal = (mode: 'add' | 'edit', data: SalesMeeting | null = null) => {
    ...
    const tableColumns = [
        ...
        },
        {
            ...
            header: t('Account'),
            sortable: true,
            render: (_: any, item: SalesMeeting) => item.account?.name || '-'
        },
        {
            key: 'start_date',
            header: t('Start Date'),
        ...
        ...(auth.user?.permissions?.some((p: string) => ['view-sales-meetings', 'edit-sales-meetings', 'delete-sales-meetings'].includes(p)) ? [{
            key: 'actions',
            header: t('Actions'),
            render: (_: any, item: SalesMeeting) => (
    ...
    return (
            ...
            <AuthenticatedLayout
                ...
                pageActions={
                    <div className="flex gap-2">
                        ...
                        {auth.user?.permissions?.includes('create-sales-meetings') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
        ...
        >
                    ...
                    <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            <ListGridToggle
                                ...
                                routeName="sales.meetings.index"
                            ...
                            <div className="relative">
                                ...
                                />
                                {(() => {
                                    const activeFilters = [filters.status, filters.parent_type, filters.account_id, filters.assigned_user_id, filters.date_range].filter(Boolean).length;
                ...
                <CardContent className="p-0">
                    {viewMode === 'list' ? (
                                ...
                                <DataTable
                                    ...
                                    emptyState={
                                        <NoRecordsFound
                                            ...
                                            hasFilters={!!(filters.name || filters.status || filters.parent_type || filters.account_id || filters.assigned_user_id || filters.date_range)}
                                            onClearFilters={clearFilters}
                                            createPermission="create-sales-meetings"
                        ...
                        <div className="overflow-auto max-h-[70vh] p-6">
                            {salesMeetings?.data?.length > 0 ? (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
                                    {salesMeetings.data.map((meeting) => (
                                        <Card key={meeting.id} className="p-0 hover:shadow-lg transition-all duration-200 relative overflow-hidden flex flex-col h-full min-w-0">
                                            {/* Header */}
                                            <div className="p-4 bg-gradient-to-r from-gray-50 to-transparent border-b flex-shrink-0">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        <h3 className="font-semibold text-sm text-gray-900">{meeting.name}</h3>
                                                        <p className="text-xs font-medium text-gray-600">{meeting.account?.name || t('No Account')}</p>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="p-4 flex-1 min-h-0">
                                                <div className="grid grid-cols-2 gap-4 mb-4">
                                                    <div className="text-xs min-w-0">
                                                        ...
                                                        <p className="font-medium text-xs">{formatDate(meeting.start_date)}</p>
                                                    </div>
                                                    <div className="text-xs min-w-0">
                                                        <p className="text-muted-foreground mb-1 text-xs uppercase tracking-wide">{t('End Date')}</p>
                                                        <p className="font-medium text-xs">{formatDate(meeting.end_date)}</p>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="grid grid-cols-2 gap-4 mb-4">
                                                    <div className="text-xs min-w-0">
                                                        ...
                                                        <span className={`px-2 py-1 rounded-full text-xs font-medium capitalize ${
                                                            meeting.meeting_type?.toLowerCase() === 'online' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'
                                                        }`}>
                                                    ...
                                                    <div className="text-xs min-w-0">
                                                        ...
                                                        <p className="font-medium text-xs capitalize">{meeting.parent_type || '-'}</p>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="grid grid-cols-2 gap-4">
                                                    <div className="text-xs min-w-0">
                                                        ...
                                                        <p className="font-medium text-xs">{meeting.assigned_user?.name || '-'}</p>
                                                    </div>
                                                    <div className="text-xs min-w-0">
                                                        <p className="text-muted-foreground mb-1 text-xs uppercase tracking-wide">{t('Created')}</p>
                                                        <p className="font-medium text-xs">{formatDate(meeting.created_at)}</p>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex justify-between items-center p-3 border-t bg-gray-50/50 flex-shrink-0 mt-auto">
                                                <span className={`px-2 py-1 rounded-full text-xs font-medium capitalize ${
                                                    ...
                                                    meeting.status?.toLowerCase() === 'in_progress' ? 'bg-yellow-100 text-yellow-800' :
                                                    ...
                                                    <div className="flex gap-1">
                                                        <TooltipProvider>
                                                            {auth.user?.permissions?.includes('view-sales-meetings') && (
                                                                <Tooltip delayDuration={0}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button variant="ghost" size="sm" onClick={() => router.get(route('sales.meetings.show', meeting.id))} className="h-8 w-8 p-0 text-green-600 hover:text-green-700">
                                                                            <Eye className="h-4 w-4" />
                                                                        </Button>
                                                                    </TooltipTrigger>
                                                                    <TooltipContent>
                                                            ...
                                                            {auth.user?.permissions?.includes('edit-sales-meetings') && (
                                                                <Tooltip delayDuration={0}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button variant="ghost" size="sm" onClick={() => openModal('edit', meeting)} className="h-8 w-8 p-0 text-blue-600 hover:text-blue-700">
                                                                            <Edit className="h-4 w-4" />
                                                                        </Button>
                                                                    </TooltipTrigger>
                                                                    <TooltipContent>
                                                            ...
                                                            {auth.user?.permissions?.includes('delete-sales-meetings') && (
                                                                <Tooltip delayDuration={0}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button variant="ghost" size="sm" onClick={() => openDeleteDialog(meeting.id)} className="h-8 w-8 p-0 text-red-600 hover:text-red-700">
                                                                            <Trash2 className="h-4 w-4" />
                                                                        </Button>
                                                                    </TooltipTrigger>
                                                                    <TooltipContent>
            ...
            <Dialog open={modalState.isOpen} onOpenChange={closeModal}>
                {modalState.mode === 'add' && (
                    <CreateSalesMeeting onSuccess={closeModal} users={users} accounts={accounts} />
                )}
                {modalState.mode === 'edit' && modalState.data && (
                    <EditSalesMeeting
                        salesMeeting={modalState.data}
