...
import { CreateSalesMeetingProps, CreateSalesMeetingFormData } from './types';
import { useState, useEffect } from 'react';

export default function Create({ onSuccess, users = [], accounts = [], defaultAccountId, defaultParentType, defaultParentId }: CreateSalesMeetingProps) {
    const { t } = useTranslation();
    const { auth } = usePage<any>().props;
    const { data, setData, post, processing, errors } = useForm<CreateSalesMeetingFormData>({
        name: '',
        status: 'scheduled',
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('sales.meetings.store'), {
            onSuccess: () => {
                onSuccess();
            }
        });
    ...
    return (
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={submit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <Label htmlFor="meeting_type">{t('Meeting Type')}</Label>
