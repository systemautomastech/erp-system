...
export default function Create() {
    ...
    const { customers, warehouses, accounts, users, opportunities, contacts, shippingProviders, quotes, fromAccount, fromOpportunity, fromContact, contactAccount } = usePage<CreateProps>().props;
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        status: 'draft',
        account_id: fromAccount ? parseInt(fromAccount) : null,
    ...
    const handleWarehouseChange = async (warehouseId: string) => {
        ...
        // Don't reset items when warehouse changes
    };
    ...
    const handleQuoteChange = async (quoteId: string) => {
        ...
        if (quoteId && quoteId !== 'none') {
            try {
                ...
                // Load warehouse products after setting data
            ...
            // Reset items when no quote selected
            setData(prev => ({
                ...prev,
                items: [{
                    product_id: 0,
        ...
        if (opportunityId && opportunityId !== 'none') {
            try {
                ...
                setData(prev => ({
                    ...
                    account_id: fromAccount || (fromAccount ? details.account_id : prev.account_id) || null
                }));
            } catch (error) {
                console.error('Failed to fetch opportunity details:', error);
            }
        } else {
            setData('opportunity_id', null);
        }
    };
    ...
    // Ensure selected contacts are always available even if not in filtered list
    ...
    const filteredQuotes = React.useMemo(() => {
        if (fromOpportunity && data.opportunity_id) {
            return quotes?.filter((quote: any) => quote.opportunity_id === data.opportunity_id);
        }
        if ((fromAccount || fromContact) && data.account_id) {
            return quotes?.filter((quote: any) => quote.account_id === data.account_id);
        }
        return quotes;
    }, [quotes, fromOpportunity, fromAccount, fromContact, data.opportunity_id, data.account_id]);
                        ...
                        <CardContent>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
                                ...
                                <div>
                                    ...
                                    <Select value={data.opportunity_id?.toString() || ''} onValueChange={handleOpportunityChange} disabled>
                                        <SelectTrigger>
                                ...
                                <div>
                                    ...
                                    <Select value={data.account_id?.toString() || ''} onValueChange={(value) => setData('account_id', value ? parseInt(value) : null)} disabled={!!fromAccount && !fromContact}>
                                        <SelectTrigger>
