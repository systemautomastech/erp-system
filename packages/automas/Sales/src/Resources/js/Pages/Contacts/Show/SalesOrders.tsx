...
export default function SalesOrders({ contact, onRegisterAddHandler }: SalesOrdersProps) {
    const { t } = useTranslation();
    const pageProps = usePage().props as any;
    const { salesOrders, auth } = pageProps;

    useEffect(() => {
        if (auth.user?.permissions?.includes('create-sales-orders')) {
            onRegisterAddHandler(() => {
                router.post(route('sales.orders.create.context'), { from_contact: contact.id });
            });
        }
    }, [onRegisterAddHandler, auth, contact.id]);
                    ...
                    <DataTable
                        ...
                        columns={[
                            {
                                ...
                                render: (value: string, order: any) =>
                                    auth.user?.permissions?.includes('view-sales-orders') ? (
                                        <span className="text-blue-600 hover:text-blue-700 cursor-pointer" onClick={() => router.get(route('sales.orders.show', order.id))}>#{value}</span>
                                    ) : (
                                        `#${value}`
                                    )
                            },
                            ...
                            },
                            {
                                ...
                                render: (_: any, item: any) => item.assignUser?.name || item.assign_user?.name || '-'
                            },
                            {
                                key: 'status',
                                header: t('Status'),
                                    ...
                                    <div className="flex gap-1">
                                        ...
                                        {auth.user?.permissions?.includes('edit-sales-orders') && (
                                            <TooltipProvider>
                                                <Tooltip delayDuration={0}>
                                                    <TooltipTrigger asChild>
                                                        <Button variant="ghost" size="sm" onClick={() => router.visit(route('sales.orders.edit', item.id))} className="h-8 w-8 p-0 text-blue-600 hover:text-blue-700">
                        ...
                        emptyState={
                            <NoRecordsFound
                                ...
                                onCreateClick={() => router.post(route('sales.orders.create.context'), { from_contact: contact.id })}
