...
import { CreateContactFormData, CreateContactProps } from './types';
import { useFormFields } from '@/hooks/useFormFields';

export default function Create({ accounts, users, onSuccess, defaultAccountId }: CreateContactProps) {
    const { t } = useTranslation();
    const { auth } = usePage<any>().props;
    const { data, setData, post, processing, errors } = useForm<CreateContactFormData>({
        name: '',
    ...
    // Custom fields hook
    const customFields = useFormFields('getCustomFields', { ...data, module: 'Sales', sub_module: 'Contact' }, setData, errors, 'create', t);

    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('sales.contacts.store'), {
            onSuccess: () => {
                onSuccess?.();
            }
        });
        ...
        <DialogContent className="max-w-2xl">
            ...
            <form onSubmit={submit} className="space-y-4">
                ...
                <div>
                    ...
                    <Select value={data.account_id} onValueChange={(value) => setData('account_id', value)} disabled={!!defaultAccountId}>
