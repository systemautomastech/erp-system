                        ...
                        <CardContent>
                            <CalendarView
                                events={calendarEvents.map(event => {
                                    ...
                                    if (event.type === 'Lead Task') {
                                        color = event.status === 'completed' ? '#10b981' : '#3b82f6'; // Green for completed, blue for pending lead tasks
                                    } else {
                                        color = event.status === 'completed' ? '#10b981' : '#f59e0b'; // Green for completed, orange for pending deal tasks
                                    }
                                    ...
                                    return {
                                        ...
                                        color,
                                        description: `${t('Task')}: ${event.title} - ${event.type === 'Lead Task' ? t('Lead') : t('Deal')}: ${event.name || ''} - ${t('Status')}: ${t(event.status?.charAt(0).toUpperCase() + event.status?.slice(1) || 'Unknown')}`,
                                        type: event.type || 'Deal Task',
                                    };
                                })}
                                onEventClick={(event) => { }}
                                onDateClick={(date) => { }}
