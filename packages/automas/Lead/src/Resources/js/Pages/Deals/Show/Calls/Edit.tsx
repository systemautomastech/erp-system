...
export default function Edit({ call, userDeals, onSuccess }: EditCallProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        setLoading(true);
        router.put(route('deal.calls.update', call.id), form, {
            onSuccess: () => onSuccess(),
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    ...
                    <Select value={form.assignee} onValueChange={(v) => setForm({ ...form, assignee: v })}>
                        ...
                        <SelectContent>
                            {userDeals?.map((ud: any) => (
                                <SelectItem key={ud.user?.id} value={ud.user?.id?.toString() || ''}>{ud.user?.name || ''}</SelectItem>
                            ))}
                        </SelectContent>
                    </Select>
                ...
                <div className="flex justify-end gap-2">
                    ...
                    <Button type="submit" disabled={loading}>{loading ? t('Updating...') : t('Update')}</Button>
                </div>
            </form>
        </DialogContent>
    );
