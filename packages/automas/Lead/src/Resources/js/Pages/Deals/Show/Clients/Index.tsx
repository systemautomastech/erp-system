...
export default function Index({ deal, availableClients, onRegisterAddHandler }: ClientsProps) {
    ...
    const columns = [
            ...
            render: (_: any, clientDeal: any) => (
                <div className="h-8 w-8 rounded-full border-2 border-background overflow-hidden">
                    {clientDeal.client?.avatar ? (
                        <img src={getImagePath(clientDeal.client.avatar)} alt={clientDeal.client.name} className="h-full w-full object-cover" />
                    ) : (
                        <div className="h-full w-full bg-primary/10 flex items-center justify-center text-sm font-medium">
                            {clientDeal.client?.name?.charAt(0)?.toUpperCase() || 'C'}
                        </div>
                    )}
                </div>
            ),
        },
        {
            ...
            render: (_: any, clientDeal: any) => clientDeal.client?.name || '-',
        },
        ...(auth?.user?.permissions?.some((p: string) => ['delete-deal-clients'].includes(p)) ? [{
            key: 'actions',
            header: t('Action'),
            render: (_: any, clientDeal: any) => (
                        ...
                        {auth?.user?.permissions?.includes('delete-deal-clients') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" onClick={() => openDeleteDialog(clientDeal.client?.id)} className="h-8 w-8 p-0 text-destructive hover:text-destructive">
    ...
    return (
        <>
            <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-medium">{t('Clients')}</h3>
