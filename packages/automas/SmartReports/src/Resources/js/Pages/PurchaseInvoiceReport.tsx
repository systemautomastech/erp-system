...
import { Filter, FileText, DollarSign, TrendingUp, ShoppingCart, Search, ChevronRight, ChevronDown, Clock, Building2, Warehouse, EyeOff, FilterX, ArrowLeft } from 'lucide-react';
...
const PurchaseInvoiceReport: React.FC = () => {
    ...
    const [reportData, setReportData] = useState<PurchaseInvoiceRecord[] | null>(null);
    const [summary, setSummary] = useState<Summary | null>(null);
    const [searchQuery, setSearchQuery] = useState('');
    const [groupBy, setGroupBy] = useState<'vendor' | 'warehouse' | 'monthly'>('vendor');
    ...
    useEffect(() => {
        ...
        const autoDateRange = `${pad(currentMonthStart)} - ${pad(currentMonthEnd)}`;
        setDateRange(autoDateRange);

        setTimeout(() => {
            const [startDate, endDate] = autoDateRange.split(' - ');
            axios.post(route('smart-reports.purchase-invoice.generate'), {
    ...
    const fetchFilterOptions = async () => {
        try {
            const response = await axios.get(route('smart-reports.purchase-invoice.filter-options'));
            if (response.data.success) {
    ...
    const handleGenerateReport = async (exportType: 'view') => {
        ...
        try {
            const response = await axios.post(route('smart-reports.purchase-invoice.generate'), {
                start_date: startDate,
                end_date: endDate,
                vendor_ids: selectedVendors.length > 0 ? selectedVendors : null,
    ...
    const buildGroups = (data: PurchaseInvoiceRecord[], mode: 'vendor' | 'warehouse' | 'monthly') => {
            ...
            <CardContent className="px-4 py-3">
                ...
                <div className="hidden xl:flex items-center gap-2">
                    ...
                    {filterOptions && (
                        <>
                            <div className="flex-1 min-w-0">
                                <Select value={selectedVendors.length > 0 ? selectedVendors[0].toString() : 'all'} onValueChange={(v) => setSelectedVendors(v === 'all' ? [] : [parseInt(v)])}>
                                    <SelectTrigger><SelectValue placeholder={t('Vendor')} /></SelectTrigger>
                                    <SelectContent>
                                        <SelectItem value="all">{t('All Vendors')}</SelectItem>
                                        {filterOptions.vendors.map(v => <SelectItem key={v.value} value={v.value.toString()}>{v.label}</SelectItem>)}
                                    </SelectContent>
                                </Select>
                            </div>
                            <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        ...
                        <TooltipProvider><Tooltip delayDuration={0}><TooltipTrigger asChild><Button size="sm" variant="outline" className="text-muted-foreground" onClick={() => { const now = new Date(); const start = new Date(now.getFullYear(), now.getMonth(), 1); const end = new Date(now.getFullYear(), now.getMonth() + 1, 0); const fmt = (d: Date) => `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`; setDateRange(`${fmt(start)} - ${fmt(end)}`); setSelectedVendors([]); setSelectedWarehouses([]); setSelectedStatus([]); axios.post(route('smart-reports.purchase-invoice.generate'), { start_date: fmt(start), end_date: fmt(end), vendor_ids: null, warehouse_ids: null, status: null, export_type: 'view' }).then(r => { if (r.data.success) { setReportData(r.data.data.data || []); setSummary(r.data.data.summary || null); } }).catch(() => {}); }}><FilterX className="h-4 w-4" /></Button></TooltipTrigger><TooltipContent><p>{t('Clear Filter')}</p></TooltipContent></Tooltip></TooltipProvider>
                    </div>
                </div>
    ...
    const pageActions = (
        <div className="flex items-center gap-2">
            <ExportButton
                ...
                filename={`purchase-invoice-report-${dateRange.replace(' - ', '-to-')}`}
                title={t('Purchase Invoice Report')}
                groupedConfig={{
                    sheetName: 'Purchase Invoice',
                    groupKey: (r) => {
                        if (groupBy === 'vendor') return r.vendor_name;
                        if (groupBy === 'warehouse') return r.warehouse_name || 'No Warehouse';
                        const d = new Date(r.invoice_date);
                        return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`;
                    },
                    summaryColumns: [
                        { key: 'group_name',     header: groupBy === 'vendor' ? t('Vendor') : groupBy === 'warehouse' ? t('Warehouse') : t('Period') },
                    ...
                    summaryBuilder: (rows) => {
                        const gName = groupBy === 'vendor' ? rows[0].vendor_name
    ...
    const breadcrumbs = [
        ...
        { label: t('Purchase Invoice Report') },
    ];

    if (!reportData || reportData.length === 0) {
        return (
            <AuthenticatedLayout breadcrumbs={breadcrumbs} pageTitle={t('Purchase Invoice Report')} pageActions={pageActions}>
                <Head title={t('Purchase Invoice Report')} />
                    ...
                    {reportData !== null && reportData.length === 0 && (
                        <Card>
                            <CardContent className="text-center py-12">
                                ...
                                <p className="text-lg font-medium text-muted-foreground">{t('No purchase invoices found.')}</p>
                            </CardContent>
                        </Card>
                    )}
                </div>
                                    ...
                                    <TooltipProvider>
                                        <Tooltip delayDuration={0}>
                                            <TooltipTrigger asChild>
                                                <Button variant={groupBy === 'vendor' ? 'default' : 'ghost'} size="sm" onClick={() => setGroupBy('vendor')} className="rounded-r-none">
                                                    <Building2 className="h-4 w-4" />
                                                </Button>
                                            </TooltipTrigger>
                                            <TooltipContent><p>{t('By Vendor')}</p></TooltipContent>
                                        </Tooltip>
                                    </TooltipProvider>
                                    <TooltipProvider>
                                        <Tooltip delayDuration={0}>
                    ...
                    <Card>
                        <CardContent className="p-4 border-b bg-gray-50/50">
                            <div className="flex items-center justify-between gap-4">
                                ...
                                <div className="flex items-center gap-2">
                                    <PerPageSelector
                                        routeName="smart-reports.purchase-invoice.index"
                                ...
                                <thead className="bg-muted/50 sticky top-0">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3 text-left font-medium">
                                            {groupBy === 'vendor' ? t('Vendor') : groupBy === 'warehouse' ? t('Warehouse') : t('Period')}
                                        </th>
                                        ...
                                        <th className="px-4 py-3 text-center font-medium">{t('Payment %')}</th>
                                    </tr>
                                </thead>
                                        ...
                                        paginatedGroups.map((group) => (
                                            <React.Fragment key={group.key}>
                                                ...
                                                {expandedGroups.has(group.key) && (() => {
                                                    ...
                                                    // 'monthly' or 'warehouse': sub-group by vendor
                                                    const vendorSubGroups = Object.values(
                                                        group.invoices.reduce((acc, inv) => {
                                                            if (!acc[inv.vendor_name]) {
                                                                acc[inv.vendor_name] = { name: inv.vendor_name, invoices: [], total: 0, paid: 0, balance: 0 };
                                                            }
                                                            acc[inv.vendor_name].invoices.push(inv);
                                                            acc[inv.vendor_name].total += +inv.total_amount;
                                                            acc[inv.vendor_name].paid += +inv.paid_amount;
                                                            acc[inv.vendor_name].balance += +inv.balance_amount;
                                                            return acc;
                                                        }, {} as Record<string, { name: string; invoices: PurchaseInvoiceRecord[]; total: number; paid: number; balance: number }>)
                                                    );
                                                                ...
                                                                <div className="px-8 py-4">
                                                                    <table className="w-full text-sm">
                                                                        <thead className="bg-white">
                                                                            <tr className="border-b">
                                                                                <th className="w-8"></th>
                                                                                <th className="px-3 py-2 text-left font-medium text-xs uppercase text-muted-foreground">{t('Vendor')}</th>
                                                                        ...
                                                                        <tbody className="divide-y">
                                                                            {vendorSubGroups.map((vg) => {
                                                                                const subKey = `${group.key}::${vg.name}`;
                                                                                const isSubExpanded = expandedGroups.has(subKey);
                                                                                const hasMultiple = vg.invoices.length > 1;
                                                                                return (
                                                                                    <React.Fragment key={subKey}>
                                                                                        {!hasMultiple ? (
                                                                                            <tr className="bg-white hover:bg-gray-50 transition-colors">
                                                                                                <td></td>
                                                                                                <td className="px-3 py-2 font-medium text-gray-800">{vg.name}<span className="ml-2 text-xs text-muted-foreground font-normal">({vg.invoices[0].invoice_number})</span></td>
                                                                                                <td className="px-3 py-2 text-center">
                                                                                                    <span className="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold bg-blue-100 text-blue-800">1</span>
                                                                                                </td>
                                                                                                <td className="px-3 py-2 text-xs text-muted-foreground whitespace-nowrap">{formatDate(vg.invoices[0].invoice_date)}</td>
                                                                                                <td className="px-3 py-2 text-right font-bold">{formatCurrency(vg.invoices[0].total_amount)}</td>
                                                                                                <td className="px-3 py-2 text-right text-green-600 font-semibold">{formatCurrency(vg.invoices[0].paid_amount)}</td>
                                                                                                <td className="px-3 py-2 text-right text-red-600 font-semibold">{formatCurrency(vg.invoices[0].balance_amount)}</td>
                                                                                                <td className="px-3 py-2 text-center">
                                                                                                    <span className={getStatusBadgeClass(vg.invoices[0].status)}>
                                                                                                        {t(vg.invoices[0].status.charAt(0).toUpperCase() + vg.invoices[0].status.slice(1))}
                                                                                                    </span>
                                                                                                </td>
                                                                                            </tr>
                                                                                        ) : (
                                                                                            <>
                                                                                                <tr className="bg-white hover:bg-blue-50/30 transition-colors cursor-pointer" onClick={() => toggleGroup(subKey)}>
                                                                                                    ...
                                                                                                    <td className="px-3 py-2 font-semibold text-gray-800">{vg.name}</td>
                                                                                                    <td className="px-3 py-2 text-center">
                                                                                                        <span className="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold bg-blue-100 text-blue-800">{vg.invoices.length}</span>
                                                                                                    </td>
                                                                                                    <td className="px-3 py-2 text-left text-xs text-muted-foreground">—</td>
                                                                                                    <td className="px-3 py-2 text-right font-bold">{formatCurrency(vg.total)}</td>
                                                                                                    <td className="px-3 py-2 text-right text-green-600 font-semibold">{formatCurrency(vg.paid)}</td>
                                                                                                    <td className="px-3 py-2 text-right text-red-600 font-semibold">{formatCurrency(vg.balance)}</td>
                                                                                                    <td></td>
                                                                                                </tr>
                                                                                                {isSubExpanded && vg.invoices.map((record, idx) => (
