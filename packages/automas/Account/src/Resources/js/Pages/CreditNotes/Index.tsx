...
export default function Index() {
    ...
    // Component for credit note action buttons
    const CreditNoteActionButtons = ({ creditNote }: { creditNote: CreditNote }) => {
        const actionButtons = usePageButtons('xeroSyncCreditNoteBtn', { credit_note_id: creditNote.id, auth, status: creditNote.status });
        ...
        ...(auth.user?.permissions?.some((p: string) => ['view-credit-notes', 'approve-credit-notes', 'delete-credit-notes'].includes(p)) ? [{
            ...
            render: (_: any, creditNote: CreditNote) => (
                <div className="flex gap-1">
                    <TooltipProvider>
<CreditNoteActionButtons creditNote={creditNote} />
