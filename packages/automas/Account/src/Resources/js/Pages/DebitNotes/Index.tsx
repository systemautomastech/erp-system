...
export default function Index() {
    ...
    // Component for debit note action buttons
    const DebitNoteActionButtons = ({ debitNote }: { debitNote: DebitNote }) => {
        const actionButtons = usePageButtons('xeroSyncDebitNoteBtn', { debit_note_id: debitNote.id, auth, status: debitNote.status });
        ...
        ...(auth.user?.permissions?.some((p: string) => ['view-debit-notes', 'approve-debit-notes', 'delete-debit-notes'].includes(p)) ? [{
            ...
            render: (_: any, debitNote: DebitNote) => (
                <div className="flex gap-1">
                    <TooltipProvider>
<DebitNoteActionButtons debitNote={debitNote} />
