...
import { EditChartOfAccountProps, EditChartOfAccountFormData } from './types';
import { usePage } from '@inertiajs/react';
import { useEffect, useState } from 'react';
import axios from 'axios';

export default function EditChartOfAccount({ chartofaccount, onSuccess }: EditChartOfAccountProps) {
    ...
    const { data, setData, put, processing, errors } = useForm<EditChartOfAccountFormData>({
        account_code: chartofaccount.account_code ?? '',
        account_name: chartofaccount.account_name ?? '',
        normal_balance: chartofaccount.normal_balance || 'debit',
        opening_balance: chartofaccount.opening_balance ?? '',
        current_balance: chartofaccount.current_balance ?? '',
        is_active: chartofaccount.is_active ?? false,
        description: chartofaccount.description ?? '',
        account_type_id: chartofaccount.account_type_id?.toString() || '',
        parent_account_id: chartofaccount.parent_account_id?.toString() || '',
    });

    const [isSubAccount, setIsSubAccount] = useState(!!chartofaccount.parent_account_id);
    ...
    return (
        <DialogContent>
            <DialogHeader>
                <DialogTitle>{t('Edit Chart Of Account')}</DialogTitle>
            </DialogHeader>
