...
export default function Create({ categories, bankAccounts, chartOfAccounts, onSuccess }: CreateExpenseProps) {
    const { t } = useTranslation();

    const { data, setData, post, processing, errors } = useForm<CreateExpenseFormData>({
    ...
    const calendarFields = useFormFields('getCalendarSyncFields', data, setData, errors, 'create', t, 'Expense');

    // AI hooks for description field
    const descriptionAI = useFormFields('aiField', data, setData, errors, 'create', 'description', 'Description', 'account', 'expense');

    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('account.expenses.store'), {
            onSuccess: () => {
                onSuccess();
            }
        });
