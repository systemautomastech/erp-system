...
interface CreateRevenueFormData {
    revenue_date: string;
    category_id: string;
...
export default function Create({ categories, bankAccounts, chartOfAccounts, onSuccess }: CreateRevenueProps) {
    ...
    const { data, setData, post, processing, errors } = useForm<CreateRevenueFormData>({
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('account.revenues.store'), {
            onSuccess: () => {
                onSuccess();
            }
        });
    ...
    return (
        <DialogContent className="max-w-2xl">
            <DialogHeader>
                <DialogTitle>{t('Create Revenue')}</DialogTitle>
            </DialogHeader>
