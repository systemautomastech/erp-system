...
class ModuleController extends Controller
{
    public function index()
    {
        if (Auth::user()->can('manage-add-on')) {
            ...
            return Inertia::render('modules/index', [
                'modules' => $modules,
                'addOns' => $addOns,
                'exploreUrl' => 'https://automas..com.bd',
        ...
        if (!empty($module)) {
            ...
            if ($module->isEnabled()) {
                ...
                if ($check_parent_module['status'] == true) {
                    ...
                    if (empty($addon)) {
                        Artisan::call('migrate --path=/packages/automas/' . $request->name . '/src/Database/Migrations --force');
                        Artisan::call('package:seed ' . $request->name);

                        $filePath = base_path('packages/automas/' . $request->name . '/module.json');
    ...
    {
        $path = base_path('packages/automas/' . $module->name) . '/module.json';
            ...
            if (File::exists($moduleJsonPath)) {
                ...
                if ($moduleData) {
                    ...
                    $modules[] = [
                        ...
                        'image' => url('/packages/automas/' . $moduleName . '/favicon.png'),
    ...
    private function installSinglePackage($file)
    {
        ...
        $tempPath = base_path('packages/automas/tmp_' . uniqid());
        ...
        $extractPath = base_path('packages/automas/' . $moduleData['name']);
            ...
            foreach ($userActiveModules as $userModule) {
                ...
                if ($addon) {
                    $modules[] = [
                        ...
                        'alias' => $addon->name,
                        'image' => url('/packages/automas/' . $userModule->module . '/favicon.png'),
